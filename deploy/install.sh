...
EXISTING_MAILING_UNSUBSCRIBE_SECRET="$(ssh "$SSH_TARGET" "if [ -f '$DEPLOY_DIR/.env' ]; then grep -E '^MAILING_UNSUBSCRIBE_SECRET=' '$DEPLOY_DIR/.env' | tail -n 1 | cut -d= -f2-; fi" || true)"
MAILING_UNSUBSCRIBE_SECRET="${MAILING_UNSUBSCRIBE_SECRET:-${EXISTING_MAILING_UNSUBSCRIBE_SECRET:-$(random_password)}}"
EXISTING_PAYMENT_CONFIG_ENCRYPTION_KEY="$(ssh "$SSH_TARGET" "if [ -f '$DEPLOY_DIR/.env' ]; then grep -E '^PAYMENT_CONFIG_ENCRYPTION_KEY=' '$DEPLOY_DIR/.env' | tail -n 1 | cut -d= -f2-; fi" || true)"
PAYMENT_CONFIG_ENCRYPTION_KEY="${PAYMENT_CONFIG_ENCRYPTION_KEY:-${EXISTING_PAYMENT_CONFIG_ENCRYPTION_KEY:-$(openssl rand -base64 32 | tr -d '\n')}}"
echo
