...
export const currentRelease = {
  version: "5.67",
  updatedAt: "26.07.2026 23:05",
  title: "Понятная оплата и современная поддержка",
  items: [
    "Кнопка рекуррентного тарифа теперь называется «Оплатить», а тип автосписания остаётся виден в описании.",
    "Статистика поддержки собрана в единую современную панель с понятными показателями.",
    "Запросы клиентов оформлены компактными карточками со статусом, цветовым маркером и быстрым переходом."
  ]
} as const;
