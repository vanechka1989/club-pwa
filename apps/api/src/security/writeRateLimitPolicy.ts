export type WriteRateLimitPolicy = { scope: "community" | "support" | "learning"; limit: number; windowMs: number };
...
export function getWriteRateLimitPolicy(method: string, path: string): WriteRateLimitPolicy | null {
  ...
  if (path.startsWith("/learning/")) return { scope: "learning", limit: 20, windowMs: 60_000 };
  return null;
}
