...
describe("release notes", () => {
  it("publishes the deployment reliability fix as version 4.97", () => {
    expect(appVersion).toBe("4.97");
    expect(releaseNotes[0]?.title).toBe("Надёжная выкладка");
    expect(releaseNotes[0]?.items.join(" ")).toContain("API и открытие самого PWA");
    expect(releaseNotes[0]?.items.join(" ")).toContain("Caddy");
    expect(releaseNotes[1]?.version).toBe("4.96");
  });
