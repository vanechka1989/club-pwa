...
describe("admin user drilldown", () => {
  ...
  it("filters users by access state", () => {
    expect(filterUsersByAccessBreakdown("inactive", users, { now }).map((entry) => entry.id)).toEqual(["restricted"]);
  ...
  it("filters users by tariff including empty tariff as future", () => {
    ...
    expect(filterUsersByTariff("future", users).map((entry) => entry.id)).toEqual(["restricted"]);
  });
});
