...
describe("admin operational panels", () => {
  it("keeps payment, integration and server screens out of the admin shell", () => {
    ...
    expect(shell).toContain('defineAsyncComponent(() => import("./AdminPaymentsPanel.vue"))');
    expect(shell).toContain('defineAsyncComponent(() => import("./AdminProjectSettingsPanel.vue"))');
    expect(shell).toContain('defineAsyncComponent(() => import("./AdminServerPanel.vue"))');
