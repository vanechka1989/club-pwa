...
const sourceComparison = computed(() => {
  ...
  return [...keys]
    .map((key) => {
      ...
      return { key, label: first?.label ?? last?.label ?? key, visits: first?.visits ?? last?.visits ?? 0, first, last };
    })
...
<template>
  <section class="acquisition" aria-label="Аналитика привлечения">
    ...
    <section class="acquisition-card">
      <div class="acquisition-card-title"><div><strong>Источники</strong><small>Первое и последнее касание на одном экране</small></div></div>
      <div v-if="sourceComparison.length" class="acquisition-source-list">
        <article v-for="source in sourceComparison.slice(0, 8)" :key="source.key">
          <header><strong>{{ source.label }}</strong><small>{{ source.visits }} визитов</small></header>
          <div class="acquisition-source-comparison">
            <span><small>Первое касание</small><b>{{ source.first?.registrations ?? 0 }} регистраций</b><em>{{ source.first?.paidUsers ?? 0 }} оплат · {{ money(source.first?.revenueRub ?? 0) }}</em></span>
            <span><small>Последнее касание</small><b>{{ source.last?.registrations ?? 0 }} регистраций</b><em>{{ source.last?.paidUsers ?? 0 }} оплат · {{ money(source.last?.revenueRub ?? 0) }}</em></span>
          </div>
        </article>
      </div>
