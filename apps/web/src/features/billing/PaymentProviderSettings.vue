...
const props = withDefaults(defineProps<{
  ...
  section?: "connection" | "catalog";
  webhookUrls?: { payment: string; subscription: string };
  busy?: boolean;
}>(), { section: "connection", busy: false });
defineEmits<{ check: []; sync: []; copy: [value: string] }>();

const visibleWebhookUrls = computed(() => props.webhookUrls ?? props.provider?.webhookUrls);
...
<template>
  <div class="provider-settings">
    ...
    <p v-if="section === 'catalog' && provider?.lastCheckError" class="provider-settings__error">
      Не удалось проверить подключение. Проверьте API-ключ.
    </p>

    <div v-if="section === 'connection' && visibleWebhookUrls" class="provider-settings__urls">
      <p class="provider-settings__hint">Добавьте оба адреса в Lava при создании webhook.</p>
      <label v-for="(value, key) in visibleWebhookUrls" :key="key">
        <span>{{ key === "payment" ? "Webhook обычной оплаты" : "Webhook рекуррентной подписки" }}</span>
    ...
    <div v-if="section === 'catalog'" class="provider-settings__actions">
      <button type="button" :disabled="busy || !provider?.secretConfigured || !provider?.webhookSecretConfigured" @click="$emit('check')">
        Проверить
      </button>
      <button type="button" :disabled="busy || !provider?.secretConfigured || !provider?.webhookSecretConfigured" @click="$emit('sync')">
