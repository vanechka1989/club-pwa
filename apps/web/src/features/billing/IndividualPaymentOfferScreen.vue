...
async function load() {
  const requestedToken = props.token;
  loading.value = true;
  error.value = "";
  data.value = null;
  try {
    const result = await getIndividualPaymentOffer(requestedToken);
    if (props.token === requestedToken) data.value = result;
  } catch {
    if (props.token === requestedToken) {
      error.value = "Ссылка недоступна. Проверьте, что вы вошли в тот аккаунт, которому она была выдана.";
    }
  } finally {
    if (props.token === requestedToken) loading.value = false;
  }
}
