...
import { createHomeworkUpload, getLessonAssessmentStatus, startLessonQuiz, submitLessonHomework, submitLessonQuiz, type LessonAssessmentStatus, type LessonQuizAttemptResponse } from "@/api/client";
...
async function startQuiz() {
  ...
  try {
    ...
    for (const question of attempt.value.questions) answers[question.id] = { selectedOptionIds: [], text: "" };
...
async function submitHomework() {
  ...
  try {
    ...
    await submitLessonHomework(props.lessonId, { submissionKey: crypto.randomUUID(), text: homeworkText.value.trim() || null, attachments });
    ...
    <template v-if="assessment.mode === 'quiz'">
      ...
      <template v-if="attempt">
        <article v-for="(question, index) in attempt.questions" :key="question.id" class="assessment-player__question">
          ...
          <textarea v-if="question.type === 'free_text'" v-model="answers[question.id]!.text" class="text-input" rows="4" placeholder="Напишите ответ"></textarea>
