...
Это рабочий план Telegram mini app для платного закрытого клуба. Его нужно обновлять по мере того, как меняются продуктовые решения и приоритеты.

## Цель Продукта

Сделать переиспользуемый шаблон Telegram mini app для закрытого клуба по подписке:

- Пользователи открывают mini app из Telegram.
- Приложение проверяет Telegram identity через подписанный `initData`.
...
- Участник может управлять профилем, оплатой и обращениями в поддержку внутри mini app.
...
- Индексы PostgreSQL должны покрывать Telegram identity, поиск подписки и списки контента.
...
- Перед запуском нужен load test основных потоков: открыть mini app, прочитать профиль, получить список категорий обучения, открыть материал и начать checkout.
...
- `jose` для JWT/session tokens вне Telegram `initData`.
...
- Telegram mini app стабильно открывается из бота.
- Telegram identity проверяется на каждом защищённом API-запросе.
...
- Нативные мобильные приложения вне Telegram.
- Продвинутая аналитика.
- Инструменты модерации community/chat.

## Разделы Продукта

Mini app должен быть организован вокруг этих разделов первого уровня:

- Профиль - Telegram identity, статус подписки, окончание доступа, заметки по аккаунту.
...
1. Сделать локальную разработку надёжной: зависимости, lockfile, миграции, seed data и mock Telegram mode.
...
- Какой платёжный провайдер будет первым: Telegram Payments, YooKassa, Stripe, CloudPayments или manual approval?
- Поддержка будет через in-app tickets или кнопку на Telegram-менеджера/чат?
- Админка живёт внутри Telegram mini app как скрытая вкладка для владельца из `OWNER_TELEGRAM_ID` и админов из базы/`ADMIN_TELEGRAM_IDS`.
- Где будут храниться фото и видео: S3-compatible storage, Garage, внешний video hosting или Telegram-hosted files?
...
- [x] Добавить проверку Telegram `initData`.
...
- [x] Добавить нижнее меню для Telegram Mini App.
...
- [x] Добавить local mock Telegram mode для разработки в обычном браузере.
- [ ] Добавить typed API error shape.
- [ ] Добавить README-раздел по настройке Telegram bot и mini app.
...
- [ ] Выбрать первый support channel: Telegram manager chat, in-app tickets или external helpdesk.
...
- [x] Решить, где живёт админка: внутри Telegram, в web или сначала в CLI scripts.
- [x] Добавить admin role model.
- [x] Добавить первую скрытую вкладку админки в mini app.
- [x] Добавить главного админа `OWNER_TELEGRAM_ID`.
- [x] Добавить управление дополнительными админами из mini app.
...
- [ ] Добавить BullMQ для email/Telegram notifications, retries, imports или scheduled work.
- [ ] Добавить S3/Garage для member files, videos или downloadable assets.
- [ ] Добавить JWT через `jose`, если появится non-Telegram web session.
...
- Telegram `initData` - основной identity proof для mini app.
